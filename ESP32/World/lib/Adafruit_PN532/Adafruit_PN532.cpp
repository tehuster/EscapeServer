...
*/
/**************************************************************************/
bool Adafruit_PN532::SAMSleep(void) {
pn532_packetbuffer[0] = 0x16;
pn532_packetbuffer[1] = 0x28; //(0x20, for SPI) (0x28 for SPI and RF detectionThe wakeup source(s) you want too use 
  ...
  while(!isready()) {
    if (timeout != 0) {
      ...
      if (timer > timeout) {
        ...
        // PN532DEBUGPRINT.print("Pin:");
        // PN532DEBUGPRINT.println(_ss);
        return false;
      }
    }
    delay(1);
...
void Adafruit_PN532::writecommand(uint8_t* cmd, uint8_t cmdlen) {
  if (_usingSPI) {
    ...
    #endif
    digitalWrite(_ss, LOW);
    delay(1);     // or whatever the delay is for waking up the board
